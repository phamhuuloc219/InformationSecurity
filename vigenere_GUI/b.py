...
def decrypt_files():
    ...
    try:
        ...
        output_file_path = os.path.join(os.path.dirname(input_file_path), "DECRYPTED_OUTPUT.DAT")
        with open(output_file_path, "w", encoding="utf-8") as outfile:
            outfile.write(plaintext)

        messagebox.showinfo("Thành công", f"Giải mã thành công! Kết quả đã được ghi vào {output_file_path}.")
...
decrypt_button = tk.Button(root, text="Giải mã file", font=("Arial", 12), command=decrypt_files, bg="lightyellow")
